...
function LgClass(){
	...
	this.version =  "";
	this.logo = "";
	this.mul = [["ENU", "English"], ["GER", "Deutsch"], ["FRA", "Français"], ["ESP", "Español"], ["ITA", "Italiano"], ["RUS", "русский"], ["ENL", "Nederlands"], ["DAM", "Danmark"], ["POL", "Polski"], ["RON", "Româna"], ["HUN", "Magyar"], ["CHS", "简体中文"], ["CHT", "繁體中文"], ["KOR", "한국어"], ["CES", "Čeština"], ["POR", "Português"]];
}
//UI类
function UIReg(){
	//Button按钮  
...
function GlobarVar(){
	...
	this.pluginW=0;
	this.pluginH=0;
	this.SetPluginPos = function(l,t,w,h){
	...
	this.ChangPage = function (page) {
	    if (1 == page) {
	        ...
	        Rfmulti();
	        for (var i = 0; i < 9; i++) {
	            if (isOpenV[i]) gDvr.VedioPlay(i, gVar_first.ip, 3);
	        ...
	        //if(gVar.nOpenPrivew)gDvr.VedioPlay(gVar.ip,4);
	        //if(gVar.nOpenTalk) gDvr.TalkCMD(1);
	        //if(gVar.nOpenAudio) gDvr.CloseAudio();

	        for (var i = 0; i < 9; i++) {
	            if (isOpenV[i]) gDvr.VedioPlay(i, gVar_first.ip, 4);
